...
for count in [20, 40, 60, 80, 100]:
    pca = PCA(n_components=count, whiten=True)
    ...
    f = open('../extractedInformation/pca'+str(count)+'/train.csv', 'w')
    for i in range(len(new_train)):
        n = [str(k) for k in new_train[i]]
        f.writelines(','.join(n)+','+str(train[1][i])+'\n')
    f.close()

    f = open('../extractedInformation/pca'+str(count)+'/dev.csv', 'w')
    for i in range(len(new_dev)):
        n = [str(k) for k in new_dev[i]]
        f.writelines(','.join(n)+','+str(dev[1][i])+'\n')
    f.close()

    f = open('../extractedInformation/pca'+str(count)+'/test.csv', 'w')
