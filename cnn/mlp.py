...
class HiddenLayer(object):
    ...
    def distance_form_1(self, y):
        m = T.sum(T.square(self.output), axis=1)
        s = T.dot(m, y) - T.dot(m, -(y-1))
        return s

    def distance_form_2(self, y):
        m = T.sum(T.square(self.output), axis=1)
        s = T.dot(m, y) + 1/(T.dot(m, -(y-1))+0.0001)
        return s/1000.0
...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=10,
             dataset='mnist.pkl.gz', batch_size=100, n_hidden=500, ordnum=None):
    """
   ...
   """
    ...
    current_best = classifier.params
    ...
    save_object(current_best, 'models/mlp_'+str(ordnum)+'.pkl')
